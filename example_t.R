...
source('GenerateCoxtcor8.R')
source('Generatelogtcor67.R')
source('Generatelogtcor81.R')
source('d011.R')
...
for(i in (seed*1 + 1):(seed*1 + 500)){
  ...
  if(setting == 1){ 
    print(setting)
    data <- GenerateCoxtcor8(N = 5000, n = 250, phi01 = -12, M = 1.6, a = 1, rate = 0.1);
  ...
  if(setting == 2){  
    print(setting)
    data <- Generatelogtcor67(N = 5000, n = 250, rate = 0.1);
    tim <- seq(1.75, 3.781, length=50);
    adl = 0.1; adh = 0.15;  
  }
  
  if(setting == 3){ 
    print(setting)
    data <- Generatelogtcor81(N = 5000, n = 250, rate = 0.1);
    tim <- seq(1.56, 3.84, length=50);
    adl = 0.08; adh = 0.15;  
  }
#########################################################################
    ###for labeled data id
...
####### for Intrinsic estimate #############
    Intr_SSL <- IntrSSL_est(time = tim, base_cov = NULL, cova_tim, cova_ct, lcen_ct, rcen_ct, xstar_all, 
