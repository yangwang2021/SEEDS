d011_new <- function(z, d, identical = rep(0, length(z)), 
  ...
  if( all(sortd == 1) ) { 
    ...
    if(influence.fun) { 
      ...
      # create a triangle matrix of   int_ti^tj  dSx/(Sy-Sz)
      ...
      #fff0 <- t(matrix((1-nodesurv),n,n))   
      ### set inf equal to 0
      fff[which(is.infinite(fff))] <-0
      F3tu <- fff * jjj
      
      # after the F function (matrix) been set, the following is the 
      ...
      sum3 <- rowSums(as.matrix(IC2tu[,statusofnode == 0]))/N  
      ...
      var <- (sum2 + sum4 + sum6 - (sum1 + sum3 + sum5)^2)/N
      
      # sum2[1075]; sum4[1075]; sum6[1075]; sum1[1075]; sum3[1075]; sum5[1075];
      # sqrt(var[1075])
