GenerateCoxbtcor8 <- function(N, n, phi01, phi02, M, a, rate){
 
  Tstar <- runif(N+n, 0, 1/2) 
  Z <- rnorm(N+n, 5, 1)
  u <- runif(N+n, 0, 1)
  Tim <- (-a * log(u) * exp(-(phi01 * Tstar + phi02 * Z)))^{1/4}
  CL <- rweibull(N+n, 1.35, 1.3)
  CR<-runif(N+n, 0, M) + CL
  ...
  dataset <- list('X' = X, 'Delta' = Delta, 'Ltime' = CL, 'Rtime' = CR, 'Xstar' = Xstar, 
                  'Deltastar' = Deltastar, 'Label_id' = Label_id, 'Z' = Z, 'Lcount' = CL_ct,
