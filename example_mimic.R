...
library(matrixcalc);library(dplyr);library(dblcens)
...
for(i in (seed*1 + 1):(seed*1 + 500)){
  ...
  if(setting == 1){ 
    print(setting)
    data <- Generatelogweiminic(N=113623, n=1613)
    tim <- seq(1.48, 3.83, length=50); 
    adl = 0.1; adh = 0.15;
  }
  ...
  Intr_SSL <- IntrSSL_est(time = tim, base_cov = NULL, cova_tim = NULL, cova_ct = NULL, lcen_ct = NULL, 
                          rcen_ct = NULL, xstar_all, deltastar_indv, label_id, lcen_all, rcen_all, 
                          obse, ldelt, rdelt, lh_labeled, lh_unlabeled, rh_labeled, rh_unlabeled, 
                          num_folds = 10)
